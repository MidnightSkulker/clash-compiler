    ...
    Nothing ->
      case primCoreId p of
        Just c  -> do
          ...
          res <- evaluateWhnf ghcEvaluator (mkApps (Var c) args)

          case collectValueApps res of
            -- If the result has a recursive call to the core id of the
            -- primitive, replace it with a neutral primitive. Otherwise clash
            -- may try and normalize the primtive (which will fail if it is
            -- polymorphic, like many clash-specific primitives)
            --
            Just (NeVar v, xs)
              |  nameOcc (varName v) == primName p
              -> do let res' = VNeu (NePrim p xs)
                    -- traceM (show (primName p) <> ": result " <> show res')
                    pure res'

            _ -> do -- traceM (show (primName p) <> ": result " <> show res)
                    pure res
