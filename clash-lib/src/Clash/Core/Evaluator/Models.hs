...
collectValueApps :: Value -> Maybe (Neutral Value, [Either Value Type])
collectValueApps (VNeu n) = Just (go [] n)
 where
  go args = \case
...
instance AsTerm Value where
  asTerm = \case
   ...
   where
    substEnv env x =
      let envTms  = fmap asTerm (lenvTerms env)
          freeIds = localFVsOfTerms (x : Map.elems envTms)
          inScope = mkInScopeSet freeIds
          subst0  = mkSubst inScope
          subst1  = extendIdSubstList subst0 (Map.toList envTms)
          subst2  = extendTvSubstList subst1 (Map.toList (lenvTypes env))
       in substTm "substEnv" subst2 x
